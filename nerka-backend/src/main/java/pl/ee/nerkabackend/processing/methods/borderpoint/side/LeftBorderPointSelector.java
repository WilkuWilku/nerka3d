...
public class LeftBorderPointSelector implements BorderPointSelector {
    ...
    public LayerPoint getBorderPoint(RawLayer rawLayer, Map<String, Object> params) {
        log.info("LeftBorderPointSelector::getBorderPoint start - layerName: {}, number: {}, index: {}",
                rawLayer.getName(), rawLayer.getLayerNumber(), rawLayer.getIndex());
        int[][] layerData = rawLayer.getData();
        for(int y=layerData[0].length-1; y>=0; y--) {
            for(int x=0; x<layerData.length; x++) {
                if(layerData[x][y] == 1) {
                    log.info("LeftBorderPointSelector::getBorderPoint end - first point: x={}, y={}", x, y);
