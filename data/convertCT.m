               ...
               if ~isempty(R{i}{j})
                   for k=1:(length(R{i}{j})-1)
                       ...
                       if ~isempty(R{i}{j}{k}) && strcmp(R{i}{j}{k+1},'nerka')
                           result = R{i}{j}{k};
                           new_output_file_name = sprintf('%s_kidney_%d-%d.json', output_file_name_base, i, kidney_layer_index); 
                       ...
                       if ~isempty(R{i}{j}{k}) && strcmp(R{i}{j}{k+1},'Cancer')
                           result = R{i}{j}{k};
                           new_output_file_name = sprintf('%s_cancer_%d-%d.json', output_file_name_base, i, cancer_layer_index); 
                           fprintf('Adding R{%d}{%d}{%d} to file %s\n', i, j, k, strcat('output/',new_output_file_name));
                           output_file = fopen(strcat('output/',new_output_file_name), 'w');
                           fprintf(output_file, jsonencode(result));
